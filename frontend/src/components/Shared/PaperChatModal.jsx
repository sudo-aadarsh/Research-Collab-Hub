...
export default function PaperChatModal({ open, onClose, paper }) {
  ...
  const { data: messages, isLoading } = useQuery({
    queryKey: ['paper-messages', paper?.id],
    queryFn: () => papersAPI.getMessages(paper.id).then(r => r.data),
    enabled: !!paper?.id && open,
    refetchInterval: 5000,
  });

  const sendMut = useMutation({
    mutationFn: () => papersAPI.postMessage(paper.id, { message: msg }),
    onSuccess: () => {
      setMsg('');
      qc.invalidateQueries(['paper-messages', paper.id]);
    },
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Chat: ${paper?.title || ''}`}>
